    ...
    this.virtio = new VirtIO(cpu,
        {
            ...
            notification:
            {
                ...
                handlers:
                    [
                        (queue_id) => {
                            // console.log(this.otherqueue);
                            let newQueue = this.otherqueue.filter(v => v.queue === queue_id)
                            if (newQueue.length === 0) {
                                return;
                            }
                            setTimeout(() => {
                                if (this.virtio.queues[queue_id].has_request()) {
                                    let pkt = newQueue.shift();

                                    let bufchain = this.virtio.queues[queue_id].pop_request();
                                    this.Send(queue_id, bufchain, new Uint8Array(pkt.data));
                                }
                            }, 1);
                        },
                        (queue_id) => {
    ...
    for (let port = 0; port < this.ports; ++port) {
        ...
        this.bus.register("virtio-console" + port + "-input-bytes", function (data) {
            ...
            if (queue.has_request()) {
                ...
                //TODO: Buffer
                thiz.otherqueue.push({ queue, data });
            }
        }, this);
