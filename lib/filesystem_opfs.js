...
/**
 * A transaction about writing to a file
 * @constructor
 * @param {number} fid 
 * @param {number} mode
 */
function Transaction(fid, mode) {
...
export function FSOpfs(storage, qidcounter) {
    ...
    // this.ino_transactions = /** @type {Record<Number,Transaction>} */ ({});
    // this.ino_transaction_cur = 0;
    ...
    this.mounts = [];
    this.initPromise = ExtendablePromise();
...
FSOpfs.prototype.EnsureHandles = async function () {
    ...
    let ff = await navigator.storage.getDirectory();

    this.OPFSRootHandle = this.OPFSRootHandle ?? await ff.getDirectoryHandle("internal-v86-1", {
        create: true
    });
...
function ExtendablePromise() {
    let r = null;
    let resolutionFunction = (v)=>{
        r(v);
    };
    let p = new Promise((resolve)=>{
        r = resolve;
    });
    return {p, resolutionFunction};
}
/**
 * @suppress {checkTypes}
 */
FSOpfs.prototype.initialize = async function () {
    ...
    for await (let [n, handle] of this.OPFSRootHandle) {
        ...
        // recover inode
        if (inodeData.nlinks === 0) {
            // don't recover good bye, rake deleted file
            try {
            await this.OPFSRootHandle.removeEntry(inodeData.fid.toString());
            await this.OPFSDataHandle.removeEntry(inodeData.fid.toString());
            } catch {}
            continue;
        }
        ...
        let a = this;
        inodeData.changeCallback = async  function () {
            
        }
        this.inodes[inodeData.fid] = inodeData;
    ...
    this.initialized = 2;
    this.initPromise.resolutionFunction(null);
}
FSOpfs.prototype.SaveSpecificInode = async function (ino) {
    if (this.initialized !== 2) {
        ...
        try {
            ...
            console.error('Error saving inode:', err);
            // Don't rethrow - let the queue continue
        }
    
}

// Separate function to get a promise that resolves when queue is drained
FSOpfs.prototype.drainQueue = async function (ino) {
    if (!ino.queue) {
...
FSOpfs.prototype.set_state = function (state) {
    // don't do anything...all state is already persisted in opfs...
};
...
FSOpfs.prototype.link_under_dir = function (parentid, idx, name) {
    ...
    this.SaveSpecificInode(parent_inode);
...
FSOpfs.prototype.PushInode = function (inode, parentid, name) {
    ...
    inode.changeCallback = async function () {
    }
    if (parentid !== -1) {
        ...
        this.SaveSpecificInode(inode.fid);
        return;
    } else {
...
FSOpfs.prototype.CreateDirectory = function (name, parentid) {
    ...
    if (parent_inode && parent_inode.direntries.get(name)){
        throw new Error("directory/file already exists. delete to replace.");
    }
...
FSOpfs.prototype.CreateFile = function (filename, parentid) {
    const parent_inode = this.inodes[parentid];
    if (parent_inode.direntries.get(filename)) {
...
FSOpfs.prototype.OpenInode = async function (id, mode) {
    ...
    // return this.ConstructTransaction(id, mode);
    /*
...
FSOpfs.prototype.CloseInode = async function (id) {
    ...
    var inode = this.inodes[id];
await this.SaveSpecificInode(inode);
...
FSOpfs.prototype.Write = async function (id, offset, count, buffer, mode=0x10) {
    ...
    if (mode & 0x10) {
        // wanna truncate
        let newSize = offset + count;
        await this.ChangeSize(id, newSize);
    }
    if (!data || this.inodedata[id].size < (offset + count)) {
        let oldSize = inode.size;
        let newSize = Math.max(offset + count, oldSize);
        if (mode & 0x10) {
            // wanna truncate
    ...
    inode.size = newSize;
        ...
        inode.mtime = new Date().getTime();
    return new Promise((resolve)=>{
 WorkerState.sendMessage('write', { offset, length: count, recvBuffer: buffer.subarray(0, count).slice().buffer, inode: id }, () => {
...
FSOpfs.prototype.Read = async function (inodeid, offset, count) {
    ...
    return new Promise((resolve) => {
        ...
        WorkerState.sendMessage("read", { offset, length: count, inode: inodeid, readBuffer }, (value, v2) => {
            ...
            resolve(new Uint8Array(value.metadata.readBuffer));
        }, count + 4);
        if (sabRoute) {
...
FSOpfs.prototype.readdir = function (inodeid) {
    ...
    for (let [name, id] of ino.direntries) {
        if (!this.inodes[id]) {
            ino.direntries.delete(name);
        }
    }
...
FSOpfs.prototype.Search = function (parentid, name) {
    ...
    if (!this.inodes[childid]) {
        parent_inode.direntries.delete(childid);
        return -1;
    }
...
FSOpfs.prototype.SearchPath = function (path, loopcount = 0) {
    ...
    for (var i = 0; i < n; i++) {
        ...
        if (id === -1) {
            ...
            return new SearchPathResult(parentid, -1, walk[i], forward_path); // the last element in the path does not exist, but the parent
        }
    }
...
FSOpfs.prototype.read_file =async function (file) {
    const p = this.SearchPath(file);
    await this.OpenInode(p.id, 0);
