...
export function Virtio9p(filesystem, cpu, bus) {
    ...
    this.msize = MAX_REPLYBUFFER_SIZE; // maximum message size
...
Virtio9p.prototype.get_state = function()
{
    ...
    state[8] = this.fids.map(function(f) { return [f.inodeid, f.type, f.uid, f.dbg_name, f.mmode]; });
    state[9] = this.fs;
...
Virtio9p.prototype.set_state = function(state)
{
    ...
    {
        return { inodeid: f[0], type: f[1], uid: f[2], dbg_name: f[3], mode: f[4] };
    });
...
Virtio9p.prototype.ReceiveRequest = async function (bufchain) {
    ...
    switch(id)
    {
        ...
        case 12: // tlopen
            ...
            var fid = req[0];
            var mode = req[1];
            // console.log("[open] fid=" + fid + ", mode=" + mode, LOG_9P);
            var idx = this.fids[fid].inodeid;
            var inode = this.fs.GetInode(idx);
            // console.log("file open " + this.fids[fid].dbg_name, LOG_9P);
            //if (inode.status === STATUS_LOADING) return;
            var ret = await this.fs.OpenInode(idx, mode);
            this.fids[fid].mmode = mode;
        ...
        case 116: // read
            ...
            if(this.fids[fid].type === FID_XATTR) {
                ...
                if(offset > inode.size)
                {
                    ...
                    // See http://ericvh.github.io/9p-rfc/rfc9p2000.Fhtml#anchor30
                    count = 0;
                }
        ...
        case 118: // write
            ...
            // console.log("[write]: fid=" + fid + " (" + filename + ") offset=" + offset + " count=" + count + " fidtype=" + this.fids[fid].type, LOG_9P);
            if(this.fids[fid].type === FID_XATTR)
            {
            ...
            else
            {
                ...
                await this.fs.Write(this.fids[fid].inodeid, offset, count, buffer.subarray(state.offset),this.fids[fid].mmode ?? 0);
            }
